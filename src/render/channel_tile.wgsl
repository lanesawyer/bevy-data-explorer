    ...
    for (var layer = 0u; layer < layers; layer++) {
        ...
        for (var component = 0u; component < 4u; component++) {
            ...
            if index < channels.count {
                color += contribution(channels.colors[index], channels.windows[index], texel[component]);
            }
        }
    }
    return vec4<f32>(srgb_to_linear(min(color, vec3<f32>(1.0))) * channels.tint.rgb, 1.0);
}
